...
import { getPlaylistTracksByPlaylistId } from '~/features/playlist/queries';
...
export const loader = async ({ params }: Route.LoaderArgs) => {
  ...
  const playlistTracks = await getPlaylistTracksByPlaylistId(Number(params.playlistId));
  return { playlistTracks };
};

export default function PlaylistPage({ loaderData }: Route.ComponentProps) {
  const navigate = useNavigate();
  const { playlistTracks } = loaderData;
  ...
  return (
    <section className="px-3 py-6 h-full">
      <header className="mb-8 flex items-center gap-4">
        ...
        <h1 className="text-4xl font-bold">{playlistTracks[0].title}</h1>
      </header>
